    ...
    <article className="group overflow-hidden rounded-2xl border bg-white transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/5">
      <div className="relative aspect-square overflow-hidden bg-zinc-50">
        <Link href={`/products/${product.slug}`} className="absolute inset-0 block">
          <Image src={product.thumbnailUrl!} alt={product.name} fill unoptimized className="object-cover transition duration-500 group-hover:scale-105" />
      ...
      <div className="p-4">
        ...
        <div className="mt-2 flex items-center gap-1 text-xs"><Star className="size-3.5 fill-amber-400 text-amber-400" /><strong>{product.rating}</strong><span className="text-muted-foreground">· Đã bán {product.soldCount}</span></div>
