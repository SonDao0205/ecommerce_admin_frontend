...
import type { StorefrontProduct } from "../data/storefront.mock";
    ...
    <div className="min-h-screen bg-[#f6f6f6] text-[#222]">
      ...
      <main className="mx-auto max-w-7xl px-4 py-5 lg:px-6">
        ...
        <section className="grid gap-8 rounded-2xl border bg-white p-4 md:p-6 lg:grid-cols-2">
          ...
          <div className="py-1">
            <p className="text-sm font-semibold text-[#ff5a1f]">{product.brand}</p>
            <h1 className="mt-2 text-3xl font-bold leading-tight">{product.name}</h1>
            <div className="mt-3 flex flex-wrap items-center gap-2 text-sm"><strong>{product.rating}</strong><span className="flex text-amber-400">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className="size-4 fill-current" />)}</span><span className="text-muted-foreground">({product.reviewCount} đánh giá)</span><span className="text-border">|</span><span className="text-muted-foreground">Đã bán {product.soldCount}</span></div>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[120px_1fr_1fr]">
              ...
              <Button variant="outline" disabled={stock <= 0} onClick={addToCart} className="h-12 border-[#ff5a1f] text-[#ff5a1f]"><ShoppingCart /> Thêm vào giỏ</Button>
              <Button disabled={stock <= 0} onClick={addToCart} className="h-12 bg-[#ff5a1f] text-white hover:bg-[#e94b13]">Mua ngay</Button>
            </div>
